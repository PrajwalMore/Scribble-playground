...
contract ERC20 {
    ...
    function transfer(address _to, uint256 _value) external returns (bool RET_0) {
        ...
        RET_0 = _original_ERC20_transfer(_to, _value);
        if (!((_v.old_0 + _v.old_1) == (_balances[_to] + _balances[msg.sender]))) {
            emit AssertionFailed("0: Transfer does not modify the sum of balances");
            assert(false);
        }
    }

    function _original_ERC20_transfer(address _to, uint256 _value) private returns (bool) {
